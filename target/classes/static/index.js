...
var selector = document.querySelector('#modal-policy-codeIMDBPolicy.core');
...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    let rules, idxPrivacySegment = 0, approvedRules = [],
        ...
        idx = 0,ruleIdx,segmentStatus=[[]],selectedAttributeName,companyName;
    ...
    getResponse().then(response => {
        ...
        document.getElementById('uploadFilesList').addEventListener(
            'click', // we want to listen for a click
            function (e) { // the e here is the event itself
        ...
        document.getElementById('singleUploadFormBtn').addEventListener(
            'click', // we want to listen for a click
            function (e) { // the e here is the event itself

                var files = document.getElementById('singleFileUploadInput').files;
                if(files.length === 0) {
                    document.getElementById('singleFileUploadError').innerHTML = "Please select a file";
                    document.getElementById('singleFileUploadError').style.display = "block";
                }
        ...
        document.addEventListener(
            ...
            function (e) { // the e here is the event itself
                ...
                if (e.target && e.target.id.includes('approve-manual-button-rule')) {
                    approvedRules = approveManualRule(manualRule,approvedRules,idxPrivacySegment);
                    updateApprovedRulesContainer(approvedRules, idxPrivacySegment);
                }
                ...
                if (e.target && e.target.id.includes('confirm-document')) {
                    let exportObj = {"companyName":companyName,
                        "approvedRules":approvedRules};
                    fetch('http://localhost:8080/save/getJSON',
                        {
                            method: "POST",
                    ...
                    confirmSingleFile(companyName,approvedRules);
                }
                if (e.target && e.target.id.includes('edit-uploaded-file')) {
                    let fileName = document.getElementById(e.target.id).parentElement.previousElementSibling.firstChild.nextSibling.textContent.split(" ")[1];
                    console.log(fileName+".json");
                    fetch('http://localhost:8080/save/setFileName',
                        {
                            method: "POST",
                            body: fileName + ".json",
                            headers: {
...
function getFilesList() {
    return fetch('http://localhost:8080/save/files',
        {
            method: "GET",
            headers: {
                'Content-Type': 'application/json',
...
function uploadedFileList(fileList){
    let html=`<table class="table table-striped table-hover" id="doc-file-list">
                                    <thead>
                                    <tr>
                                        <th scope="col"><span>File name</span>
                                        </th>
    ...
    for (let file of fileList) {
        ...
        html+=`<tr>
                                        ...
                                        </svg></a><span>${" " + file.split(".")[0] + " privacy policy"}</span>
                                        </td>
                                        <td>
                                        <a data-toggle="modal" id="delete-uploaded-file-${idxFile}" href="#modal-container-delete">
                                            ...
                                            <a data-toggle="modal" id="edit-uploaded-file-${idxFile}" href="#modal-container-edit">
                                        ...
                                        <td><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill svg-event" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
    ...
    document.getElementById('doc-file-list').innerHTML = html;

}
function confirmSingleFile(fileName,approvedRules) {

    if(approvedRules.length) {
        document.getElementById('singleFileConfirmError').style.display = "none";
        document.getElementById('singleFileConfirmSuccess').innerHTML =  `<h6>Confirmed file was Generated Successfully.</h6><h6> DownloadUrl : <a href='${"/save/downloadFile/" + fileName.split(".")[0] + ".policy.ttl"}' target='_blank'>${fileName.split(".")[0]+ " Policy"}</a></h6>`;
        document.getElementById('singleFileConfirmSuccess').style.display = "block";
    } else {
        document.getElementById('singleFileConfirmSuccess').style.display = "none";
        document.getElementById('singleFileConfirmError').innerHTML = `<h6>Please review the documents segments before confirming the document.<h6>`;
    }
}
...
function uploadSingleFile(file) {
    ...
    xhr.open("POST", "/save/uploadFile");

    xhr.onload = function() {
        console.log(xhr.responseText);
        var response = JSON.parse(xhr.responseText);
        if(xhr.status == 200) {
            document.getElementById('singleFileUploadError').style.display = "none";
            document.getElementById('singleFileUploadSuccess').innerHTML = "<p>File Uploaded Successfully.</p>";
            document.getElementById('singleFileUploadSuccess').style.display = "block";
        } else {
            document.getElementById('singleFileUploadSuccess').style.display = "none";
...
function approveManualRule(manualRule,approvedRulesList,idxPrivacySegment){
...
function addManualRule(selectedAttributeNode,selectedAttributeValueNode,idxPrivacySegment,manualRule){
    ...
    if (typeof  manualRule[idxPrivacySegment]=="undefined"){
        ...
        if (manualRule[idxPrivacySegment][attrObjInv[selectedAttributeNode.text]].includes("") && selectedAttributeNode.text.includes("rule") ){
            console.log("here");
            manualRule[idxPrivacySegment][attrObjInv[selectedAttributeNode.text]] = selectedAttributeValueNode.text
        }
        else if (!manualRule[idxPrivacySegment][attrObjInv[selectedAttributeNode.text]].length && !selectedAttributeNode.text.includes("rule")  )
        {
            manualRule[idxPrivacySegment][attrObjInv[selectedAttributeNode.text]] = [selectedAttributeValueNode.text];
        }
...
function initDocumentTitle(data) {
    document.getElementById('document-title').innerHTML = `<h1 align="center" id="document-title">${data.companyName} policy
                                                     <a href=${data.companyUrl} type="button">
                                                    ...
                                                    <button data-toggle="modal" href="#modal-container-confirm-document" type="button" class="btn btn-outline-primary" id="confirm-document">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-square-fill svg-event" viewBox="0 0 16 16">
                                                      <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>
                                                    </svg>
                                                    </button>
                                                      </h1>
                <div aria-hidden="true" aria-labelledby="confirmDoc" class="modal fade" id="modal-container-confirm-document"
                     role="dialog">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="confirmDoc">
                                    Confirm document
                                </h5>
                                <button class="close" data-dismiss="modal" type="button">
                                    <span aria-hidden="true">×</span>
                            ...
                            <div class="modal-body">
                                    <div id="singleFileConfirmError"></div>
                                    <div id="singleFileConfirmSuccess"></div>
                                </div>
                            </div>
                        </div>
                    </div>`
